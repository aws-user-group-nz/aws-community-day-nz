...
(function ($, document, window, undefined) {
  ...
  $.fn.jCounter = function (options, callback) {
    ...
    if (
      thisEl.data("userOptions").debugLog == true &&
    ...
    if (thisEl.data("userOptions").customDuration) {
      if (!isNaN(thisEl.data("userOptions").customDuration)) {
        ...
        if (consoleLog) {
          console.log(
            "(jC) Error: The customDuration value is not a number! NOTE: 'customDuration' accepts a number of seconds."
          );
        }
      }
    }
    ...
    if (thisEl.data("userOptions").customRange) {
      ...
      if (!isNaN(rangeVal0) && !isNaN(rangeVal1)) {
        ...
        if (consoleLog) {
          console.log(
            "(jC) Error: The customRange value is not a valid range! Example: customRange: '0:30' or '30:0'"
          );
        }
      }
    }
    ...
    function initCounter(el) {
      if (customDuration) {
        ...
        $(el).data(
          ...
          setInterval(function () {
            if (endCounter == false) {
              var ifRangeDownCount = customRangeDownCount
                ? (currentTime = parseInt(currentTime) - 1)
                : (currentTime = parseInt(currentTime) + 1);
              countdown_proc(currentTime, el);
            }
        ...
        dateSource =
          thisEl.data("userOptions").serverDateSource +
          "?timezone=" +
          thisEl.data("userOptions").timezone +
          "&callback=?";
        $.ajax({
          url: dateSource,
          dataType: "json",
          ...
          error: function () {
            if (consoleLog) {
              console.log(
                "(jC) Error: Couldn't find dateandtime.php from serverDateSource: " +
                  thisEl.data("userOptions").serverDateSource +
    ...
    function jCslider(el, unitClass, timeUnit, eventDate, duration) {
      $(el)
        ...
        .each(function (i, el) {
          var twoDigits =
            thisEl.data("userOptions").twoDigits == "on" ? "0" : "";
          var newIndex =
            jCounterDirection == "up" ? (newIndex = -i) : (newIndex = i);
          currNo = parseInt(timeUnit, 10) + newIndex;
          if (String(parseInt(timeUnit, 10)).length >= 2) {
            $(el).text(parseInt(timeUnit, 10) + newIndex);
          } else if (
            String(parseInt(timeUnit, 10)).length == 1 &&
            currNo == 10
          ) {
            $(el).text(parseInt(timeUnit, 10) + newIndex);
          } else {
    ...
    function resetHTMLCounter(el) {
      ...
      if (thisEl.data("jCanimation") == "slide") {
        $(el)
          .find(
            ".daysSlider u,.hoursSlider u,.minutesSlider u,.secondsSlider u"
          )
          .text("");
      }
    }
    ...
    function countdown_proc(duration, el) {
      ...
      if (!isNaN(eventDate)) {
        ...
        if (consoleLog) {
          console.log(
            "(jC) Error: Invalid date! Here's an example: 01 January 1970 12:00:00"
          );
        }
        clearInterval($(el).data("jC_interval"));
      }
    ...
    if (jC_methods[this.options]) {
      return jC_methods[this.options].apply(
        this,
      ...
      console.log(
        "(jC) Error: Method >>> " + this.options + " <<< does not exist."
      );
    }
  };
  //the end;
